...
- **Routine step advancement & greeting:** `routine_manager._build_step_announcement` picks "Morning/Afternoon/Evening" from wall-clock hour (4-11/12-16/else) — no longer hardcoded to "Morning". `selfcare_log` fires `_maybe_advance_routine_for_action` as a fire-and-forget task; if the active routine's current step matches the logged action (keyword map for medication/meal/water/movement), it calls `advance_step("done")`. Prevents the 2026-04-17 evening-routine-stuck-all-night class of bug where non-skippable steps nudged forever: `_deliver_nudge` now force-ends the routine once `nudge_count > ROUTINE_NUDGE_MAX` on a non-skippable step (or any step when `ROUTINE_AUTO_SKIP=off`), logged at WARNING. Metrics gap: no `bgw_routine_*` or `bgw_selfcare_*` counters exist yet — a `bgw_routine_auto_ended_total{routine,step}` would let Grafana alert on stuck routines instead of relying on user notice.
