...
| `orchestrator/routes_config.py` | `/api/config/*` REST surface for the `/settings` page (Identity, Selfcare, Quiet Hours, Routines, Speakers, Recurring Reminders) + read-only `GET /api/config/features` (`{workouts_enabled, meals_enabled, jess_advanced}`) consumed by the dashboard nav to hide disabled-feature links |
