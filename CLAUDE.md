...
- **Real streaming:** `/v1/chat/completions` with `stream=true` relays vLLM tokens through SSE while the unified tool loop runs (was: `cloud_brain._stream_text` chunked the finished string). Every model round streams via `OpenAICompatibleBackend.stream_chat_completion` (now accepts tools/extra_body); `unified_loop.StreamGate` suppresses `<think>`/`<tool_call>` blocks with partial-tag holdback (vLLM's reasoning parser routes thinking to `delta.reasoning_content`, gate is the defensive second layer — XML-fallback tool calls DO arrive in content); tool_calls assemble from index-keyed deltas and are only trusted from a cleanly-finished stream. Per-round buffered fallback (`call_model`) when the backend can't stream or a stream dies pre-emission; text from buffered rounds still reaches the client via the loop's `_finalize`. Multi-round emissions join with a paragraph break. `cloud_brain._stream_loop_response` runs the loop as a producer task relaying through a **bounded** asyncio.Queue (`maxsize=64`, so a stalled consumer backpressures the producer); loop failure with nothing emitted falls back to the fallback model (buffered, run on a `messages` snapshot taken before the primary loop mutated the list, and carrying `is_voice`), failure after emission closes the stream cleanly on the partial text; client disconnect cancels the producer **and awaits it** so a mid-flight tool unwinds before teardown. Skipped on the first-ever chat (welcome-prepend ordering) and killable via `REAL_STREAMING_ENABLED=false` (read at process start — flipping needs a restart, not a hot toggle). Anthropic/OpenAI fallback backends, HA Assist + Telegram (`stream=false`), fast-path, and brain-asleep replies keep the buffered path. Metrics: `bgw_chat_ttft_seconds` (TTFT) + `bgw_chat_stream_outcome_total{outcome}` (degradation-only: `not_stream_capable`/`pre_emission_retry`/`died_mid_emission`/`loop_failed_fallback`/`loop_failed_error`; happy path uncounted, denominator = TTFT count). Grafana: TTFT + degradation panels in the "Request Flow" row (`brain_gateway_sre.py`); `ChatStreamTruncating` warning alert on `died_mid_emission` → non-paging `pushover-quiet`. Known follow-up: `StreamGate`/`clean_response` match only literal `<think>`/`<tool_call>` — an attributed tag (`<think reason=1>`) bypasses suppression (pre-existing, both paths; harden together). Tests: `tests/test_real_streaming.py`.
