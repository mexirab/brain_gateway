...
| `REAL_STREAMING_ENABLED` | `true`/`false` (default true). Real token streaming on `/v1/chat/completions`: gate-safe tokens relay through SSE while the unified tool loop runs (vLLM primary only — Anthropic/OpenAI fallbacks stay buffered, as do HA Assist and Telegram which request `stream=false`). Kill switch: `false` restores the old chunk-the-finished-string behavior. Metric: `bgw_chat_ttft_seconds`. |
