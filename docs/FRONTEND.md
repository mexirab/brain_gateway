...
**Partial — slices 1–6 of a multi-step wizard.** Welcome / Identity / Model / Voice / Push / Selfcare / Review exist. Slice 3 (2026-05-21, backend) added the env-overrides writer at `/api/setup/env` + `/api/setup/env/validate` (`orchestrator/setup_env.py`, persists to `/app/data/setup_overrides.env`). Slice 4 (2026-05-21, frontend) — `ModelStep.tsx` writes `VLLM_*` + `MODEL_NAME`. Slice 5 (2026-05-22, frontend) — `VoiceStep.tsx` writes `TTS_VOICE`. Slice 6 (2026-05-22, frontend) — `PushStep.tsx` is the first wizard slice to exercise validate-before-persist end-to-end: two independent channels (ntfy + Pushover) with per-channel Test buttons that call `POST /api/setup/env/validate`. Remaining wizard step (Optional-integrations) will POST through the same surface.
