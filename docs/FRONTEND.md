...
**Partial — slices 1–5 of a multi-step wizard.** Welcome / Identity / Model / Voice / Selfcare / Review exist. Slice 3 (2026-05-21, backend) added the env-overrides writer at `/api/setup/env` + `/api/setup/env/validate` (`orchestrator/setup_env.py`, persists to `/app/data/setup_overrides.env`). Slice 4 (2026-05-21, frontend) — `ModelStep.tsx` writes `VLLM_*` + `MODEL_NAME`. Slice 5 (2026-05-22, frontend) — `VoiceStep.tsx` writes `TTS_VOICE` (free-text, no live validation — voices depend on the picked TTS model and the server may not be reachable yet). Remaining wizard steps (Push-channel, Optional-integrations) will POST through the same surface.
