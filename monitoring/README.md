...
| Helios (`promtail-helios` sidecar in `docker-compose.yml`) | Helios Docker socket + systemd journal (whitelisted units only: `vllm-primary`, `llama-server` (disabled, retained for rollback), `llama-server-coder`, `qwen-tts`, `parakeet-stt`, `brain-gateway`) | Loki on Jupiter via `LOKI_PUSH_URL` over Tailscale MagicDNS | `host=helios` (static) |
