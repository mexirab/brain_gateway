...
async def run_self_audit() -> dict[str, Any]:
    ...
    if not settings.self_audit_enabled:
        ...
        return {"result": "skipped", "reason": "SELF_AUDIT_ENABLED=false"}
...
# ===========================================================================
# 7-day calibration review (one-shot, fires 2026-05-02)
# ===========================================================================
#
# Evaluates whether the daily F-014 audit is earning its keep. Reads the
# last 7 daily reports, pulls Loki + Prometheus ground-truth for the same
# window, and asks the *code agent* (Qwen3-Coder-Next, NOT Jess — avoids
# self-grading) to render a KEEP / TUNE / RECONSIDER verdict.
#
# Implemented in this file rather than a sibling module because it shares
# all the helpers (Loki client, output dir, push digest) and is single-shot
# enough that a separate module would be ceremony.


_REVIEW_PROMPT = """You are a senior systems engineer doing a 7-day calibration review of a daily error-log audit feature (F-014) running on a personal AI assistant deployment.

The audit job runs every morning, queries Loki for the last 24h of error/warn logs across services, asks Jess (Qwen3.5-27B) to diagnose each cluster, and pushes a Pushover digest. The user wants to know whether to KEEP, TUNE, or RECONSIDER it after 7 days of operation.

You have THREE inputs:

1. **Daily reports** — up to 7 markdown reports Jess produced. Some may be missing if the cron was down or the service wasn't running yet on day 1.
2. **Ground truth from Loki** — the actual error/warn cluster counts for the same 7-day window, queried with the exact LogQL the audit uses.
3. **Prometheus metrics** — `bgw_self_audit_runs_total{{result}}`, `bgw_self_audit_clusters_total{{severity}}`, `bgw_self_audit_format_drift_total`, latency histogram aggregated over the window.

Score Jess on:
- **Coverage**: did her cluster count match Loki ground truth, or did she miss things?
- **Severity calibration**: were her CRITICAL/HIGH/MEDIUM/LOW calls defensible?
- **Suggestion quality**: were her INVESTIGATE/FIX commands relevant or generic boilerplate?
- **Format discipline**: did the output stay in the prescribed shape, or drift?
- **False-positive rate**: how often did she flag noise as actionable?

Output (under 400 words, markdown):

## Verdict
**KEEP** | **TUNE** | **RECONSIDER**

## Coverage
<one paragraph on cluster-count match / mismatch>
...
## Format discipline
<one line on drift count + observed vs expected>

## Specific recommendations
<bullet list of 1-5 concrete prompt or filter changes if TUNE; or "none" if KEEP; or rationale for shutdown if RECONSIDER>

---

INPUTS:

### Reports found ({n_reports} of 7 expected)
Missing dates: {missing_dates}

{reports_block}

### Loki ground truth ({loki_total_clusters} clusters, 7-day window)
{loki_block}
...
async def _query_loki_summary(
    *,
    ...
    """7-day cluster-count summary from Loki, by service.

    Returns ``{"total": int, "by_service": {svc: count, ...}, "ok": bool}``.
    On transport failure returns ``{"ok": False}`` with empty counts.
    """
    query = '{host="helios", level=~"error|warn|critical|fatal"}'
    url = loki_url.rstrip("/") + "/loki/api/v1/query_range"
    params = {
    ...
    try:
        ...
        logger.exception("[SELF_AUDIT_REVIEW] Loki summary query failed")
        return {"total": 0, "by_service": {}, "ok": False}
    ...
    for stream in streams:
        ...
        total += n
    return {"total": total, "by_service": by_service, "ok": True}


async def _query_prometheus_summary(
    *,
    prom_url: str,
    days: int = 7,
    timeout_sec: int = 15,
) -> dict[str, Any]:
    """Aggregate audit metrics from Prometheus over a window.

    Best-effort — returns ``{"ok": False}`` on transport failures.
    """
    base = prom_url.rstrip("/") + "/api/v1/query"
    queries = {
        "runs_by_result": f'sum by (result) (increase(bgw_self_audit_runs_total[{days}d]))',
        "clusters_by_severity": f'sum by (severity) (increase(bgw_self_audit_clusters_total[{days}d]))',
        "format_drift": f'increase(bgw_self_audit_format_drift_total[{days}d])',
        "latency_p50": f'histogram_quantile(0.5, sum by (le) (rate(bgw_self_audit_latency_seconds_bucket[{days}d])))',
        "latency_p95": f'histogram_quantile(0.95, sum by (le) (rate(bgw_self_audit_latency_seconds_bucket[{days}d])))',
    }
    out: dict[str, Any] = {"ok": True, "results": {}}
    try:
        async with httpx.AsyncClient(timeout=timeout_sec) as client:
            for name, q in queries.items():
                r = await client.get(base, params={"query": q})
                r.raise_for_status()
                data = r.json()
                out["results"][name] = (data.get("data") or {}).get("result") or []
    except (httpx.HTTPError, json.JSONDecodeError, ValueError):
        logger.exception("[SELF_AUDIT_REVIEW] Prometheus query failed")
        return {"ok": False, "results": {}}
    return out


def _format_loki_block(summary: dict[str, Any]) -> str:
    if not summary.get("ok"):
        return "(Loki unreachable during review — coverage check inconclusive.)"
    by_svc = summary.get("by_service", {})
    if not by_svc:
        return "(No error/warn entries in the 7-day window — genuinely clean week, or scrape is broken.)"
    lines = [f"- {svc}: {n}" for svc, n in sorted(by_svc.items(), key=lambda kv: -kv[1])]
    return "\n".join(lines)


def _format_prom_block(summary: dict[str, Any]) -> str:
    if not summary.get("ok"):
        return "(Prometheus unreachable during review.)"
    res = summary.get("results", {})
    parts: list[str] = []
    for r in res.get("runs_by_result", []):
        result_label = (r.get("metric") or {}).get("result", "?")
        value = (r.get("value") or [None, "0"])[1]
        parts.append(f"- runs[{result_label}]: {value}")
    for r in res.get("clusters_by_severity", []):
        sev = (r.get("metric") or {}).get("severity", "?")
        value = (r.get("value") or [None, "0"])[1]
        parts.append(f"- clusters[{sev}]: {value}")
    for r in res.get("format_drift", []):
        value = (r.get("value") or [None, "0"])[1]
        parts.append(f"- format_drift: {value}")
    for label in ("latency_p50", "latency_p95"):
        for r in res.get(label, []):
            value = (r.get("value") or [None, "0"])[1]
            parts.append(f"- {label}: {value}s")
    return "\n".join(parts) if parts else "(No metric samples found.)"


def _read_recent_reports(
    output_dir: str, end_date: datetime, days: int = 7
) -> tuple[list[tuple[str, str]], list[str]]:
    """Read up to ``days`` daily reports ending on ``end_date`` (inclusive).

    Returns ``(found, missing)``. Each ``found`` entry is ``(YYYY-MM-DD, content)``.
    """
    out_dir = Path(output_dir)
    found: list[tuple[str, str]] = []
    missing: list[str] = []
    for offset in range(days):
        d = (end_date - timedelta(days=days - 1 - offset)).strftime("%Y-%m-%d")
        path = out_dir / f"{d}.md"
        if not path.exists():
            missing.append(d)
            continue
...
def _save_review_report_sync(
    *,
    ...
    try:
        out = Path(output_dir)
        out.mkdir(parents=True, exist_ok=True)
        path = out / f"REVIEW-{end_date.strftime('%Y-%m-%d')}.md"
        tmp = path.with_suffix(".md.tmp")
        tmp.write_text(body, encoding="utf-8")
        tmp.replace(path)
        return str(path)
    except (OSError, UnicodeError):
        logger.exception("[SELF_AUDIT_REVIEW] Failed to write review report")
        return None


async def run_self_audit_review() -> dict[str, Any]:
    """One-shot 7-day calibration review. Never raises.

    Reads up to 7 daily F-014 reports + Loki/Prometheus ground-truth, asks
    the code-agent model (Qwen3-Coder-Next) for a KEEP/TUNE/RECONSIDER
    verdict, saves the analysis to ``REVIEW-<date>.md`` next to the daily
    reports, and pushes a one-line Pushover digest.
    """
    from orchestrator.orchestrator import call_model

    if not settings.self_audit_enabled:
        logger.info("[SELF_AUDIT_REVIEW] Skipped — SELF_AUDIT_ENABLED=false")
        return {"result": "skipped", "reason": "feature flag off"}

    end_date = _now_utc()
    start_date = end_date - timedelta(days=7)

    reports, missing = await asyncio.to_thread(
        _read_recent_reports, settings.self_audit_output_dir, end_date, 7
    )
    logger.info(
        "[SELF_AUDIT_REVIEW] Reports found=%d missing=%s",
        len(reports), missing,
    )

    loki = await _query_loki_summary(
        loki_url=settings.self_audit_loki_url,
        start=start_date,
        end=end_date,
    )
    prom = await _query_prometheus_summary(
        prom_url="http://10.0.0.248:9090", days=7,
    )

    reports_block = "\n\n---\n\n".join(
        f"#### {date}\n\n{text}" for date, text in reports
    ) if reports else "(no reports found)"

    prompt = _REVIEW_PROMPT.format(
        n_reports=len(reports),
        missing_dates=", ".join(missing) if missing else "none",
        reports_block=reports_block,
        loki_total_clusters=loki.get("total", 0),
        loki_block=_format_loki_block(loki),
        prom_block=_format_prom_block(prom),
    )

    # Code agent (Qwen3-Coder-Next) is the judge — neutral, not the model
    # being graded. Falls back to disabled-flag check inside call_model.
    if not (settings.code_agent_enabled and settings.code_agent_model_url):
        logger.warning("[SELF_AUDIT_REVIEW] code_agent disabled — using Jess as fallback judge")
        judge_url, judge_model = shared.MODEL_URL, shared.MODEL_NAME
    else:
        judge_url, judge_model = settings.code_agent_model_url, settings.code_agent_model_name
    ...
    try:
        ...
        if isinstance(resp, dict):
            diagnosis = (resp.get("choices") or [{}])[0].get("message", {}).get("content", "") or ""
    except Exception:
        logger.exception("[SELF_AUDIT_REVIEW] Judge LLM call failed")

    # Build the on-disk report
    header = (
        f"# F-014 self-audit · 7-day review · {end_date.strftime('%Y-%m-%d')}\n\n"
        f"- Generated: {end_date.isoformat()}\n"
        f"- Reports analyzed: {len(reports)} of 7 (missing: {', '.join(missing) or 'none'})\n"
        f"- Loki transport: {'ok' if loki.get('ok') else 'failed'}\n"
        f"- Prometheus transport: {'ok' if prom.get('ok') else 'failed'}\n"
        f"- Judge model: {judge_model}\n\n---\n\n"
    )
    body = diagnosis or "_(Judge LLM unavailable. Raw inputs below.)_\n\n"
    if not diagnosis:
        body += f"### Loki summary\n{_format_loki_block(loki)}\n\n"
        body += f"### Prometheus summary\n{_format_prom_block(prom)}\n\n"
        body += f"### Reports\n{reports_block}\n"

    report_path = await asyncio.to_thread(
        _save_review_report_sync,
        output_dir=settings.self_audit_output_dir,
        body=header + _redact_secrets(body),
        end_date=end_date,
    )

    # Pushover digest — extract verdict line from diagnosis if present
    verdict_match = re.search(
        r"\*\*(KEEP|TUNE|RECONSIDER)\*\*", diagnosis, re.IGNORECASE
    )
    verdict = verdict_match.group(1).upper() if verdict_match else "INCONCLUSIVE"
    title = f"F-014 7-day review · {verdict}"
    msg_lines = [
        f"Reports: {len(reports)}/7" + (f" (missing {len(missing)})" if missing else ""),
        f"Loki ground-truth: {loki.get('total', 0)} clusters",
    ]
    if report_path:
        msg_lines.append(f"Report: {Path(report_path).name}")
    try:
        from orchestrator.pushover_manager import deliver_pushover_confirm
        await deliver_pushover_confirm(title=title, message="\n".join(msg_lines))
    except Exception:
        logger.warning("[SELF_AUDIT_REVIEW] Pushover digest failed (non-fatal)", exc_info=True)

    logger.info(
        "[SELF_AUDIT_REVIEW] Done · verdict=%s · reports=%d · path=%s",
        verdict, len(reports), report_path,
    )
    return {
        "result": "ok",
        "verdict": verdict,
