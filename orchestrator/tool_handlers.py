...
async def deliver_reminder_job(
    ...
    if voice_ok or phone_ok:
        ...
        await asyncio.to_thread(mark_reminder_failed, reminder_id)
        logger.error(
...
# NOTE: A previous version of this file shipped 4 configure_* chat tools
# (configure_home_assistant, configure_ntfy, configure_pushover,
# configure_paperless) that let Jess walk users through wiring up
# integrations. They were removed in the immediate follow-up because:
# (1) the validator dial leaked credentials to operator-supplied URLs
#     (live PoC: prompt injection → HA_URL → attacker host captures bearer)
# (2) the "no restart" path lied — Settings() singleton wasn't re-evaluated
# (3) they bypassed the HTTP /api/setup/env kill switch, making post-setup
#     env writes reachable via prompt injection
# The path forward (v1.1): operator side-channel confirmation (Pushover/ntfy
# tap-to-approve) gated chat tools — or stay with the /settings page which
# is already the secure post-install configuration surface.
