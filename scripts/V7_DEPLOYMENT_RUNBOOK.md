...
> **⚠️ HISTORICAL DOC.** This was the one-time runbook for migrating from v6 hybrid to v7 unified back when the stack lived on Jupiter. The stack has since been migrated to Helios (`/opt/gateway_mvp/`) and the primary model is running as Qwen3.5-27B on GPU1 (RTX PRO 5000). Keep as reference only; do not execute.
